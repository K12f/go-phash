...
//缩小DCT 虽然DCT的结果是32*32大小的矩阵，但我们只要保留左上角的32*32的矩阵，这部分呈现了图片中的最低频率。
//计算平均值 如同均值哈希一样，计算DCT的均值。
//计算hash值 根据32*32的DCT矩阵，设置0或1的64位的hash值，大于等于DCT均值的设为”1”，小于DCT均值的设为“0”。组合在一起，就构成了一个64位的整数，这就是这张图片的指纹

func TransMa() [32][32]float64 {
	var ma [32][32]float64
...
func DCT(dctMa [32][32]float64, dctMap [32][32]float64) [32][32]float64 {
	var l = len(dctMa)
	var t = 0.0
	var dctMapTemp [32][32]float64
...
func compare(average float64, dctMap [32][32]float64) (hash []int) {
	for i := 0; i < 8; i++ {
		for j := 0; j < 8; j++ {
